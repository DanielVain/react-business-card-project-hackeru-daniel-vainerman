...
router.put('/:id', auth, async (req, res) => {
	try {
		...
		if (!user.biz) {
			console.log(
				chalk.redBright(
					'A non-business user attempted to create a card!'
				)
			);
		...
		if (!card) {
			console.log(
				chalk.redBright('No card with this ID in the database!')
			);
			return res
				.status(404)
				.send('No card with this ID in the database!');
		}
...
router.delete('/:id', auth, async (req, res) => {
	try {
		...
		if (!card) {
			...
			return res
				.status(403)
				.send('You are noe authorize to delete cards');
		}
...
router.get('/liked-cards', auth, async (req, res) => {
	try {
		...
		console.log(chalk.redBright('Could not get liked cards ', err.message));
